...
:/*m_pGrenade(NULL),m_pItem(NULL),*/m_pPickUpItem(NULL),m_pMPChatWnd(NULL),UIArtefactIcon(NULL),m_pMPLogWnd(NULL),m_admin_spawner_wnd(NULL)
{
...
void CUIMainIngameWnd::Init()
{
	...
	m_admin_spawner_wnd = xr_new<CUIAdminSpawner>();
	m_admin_spawner_wnd->Init();
	AttachChild(m_admin_spawner_wnd);
	m_admin_spawner_wnd->HideDialog();
...
bool CUIMainIngameWnd::OnKeyboardAction(int dik, EUIMessages keyboard_action)
{
	if (keyboard_action == WINDOW_KEY_PRESSED)
	{
		if (dik == DIK_F10) // Example hotkey: F10
		{
			// TODO: Add admin rights check here
			ToggleAdminSpawner(!m_admin_spawner_wnd->IsShown());
			return true;
		}
	}
...
void CUIMainIngameWnd::ToggleAdminSpawner(bool bShow)
{
	...
	if (!ps || !ps->testFlag(GAME_PLAYER_SUPER_ADMIN))
	{
		// If not admin, always hide
		if (m_admin_spawner_wnd->IsShown())
		{
			m_admin_spawner_wnd->HideDialog();
		}
	...
	if (bShow)
	{
		m_admin_spawner_wnd->ShowDialog(true);
	}
	else
	{
		m_admin_spawner_wnd->HideDialog();
...
bool CUIMainIngameWnd::IsAdminSpawnerShown() const
{
	return m_admin_spawner_wnd && m_admin_spawner_wnd->IsShown();
}
